...
public static class ImageEffectPresetSerializer
{
    ...
    public static void SaveXsieFile(string filePath, string presetName, IEnumerable<ImageEffect> effects)
    {
        ...
        {
            Name = string.IsNullOrWhiteSpace(presetName) ? "Preset" : presetName,
            Effects = effects?.ToList() ?? new List<ImageEffect>()
        };
    ...
    public static XsiePreset? LoadXsieFile(string filePath)
    {
        ...
        return JsonConvert.DeserializeObject<XsiePreset>(configJson, CreateSerializerSettings());
    }
...
public sealed class XsiePreset
{
...
internal sealed class ImageEffectSerializationBinder : ISerializationBinder
{
    public Type BindToType(string? assemblyName, string typeName)
    {
        ...
        if (!typeName.StartsWith("ShareX.Editor.ImageEffects.", StringComparison.Ordinal))
